'use strict';

angular.module('mockedSites', [])
  .value('defaultSitesJSON', [{
    'id': 162,
    'footprint': [
      [
        [
          [296254.482930933, 4633726.19264522],
          [296247.357277832, 4633735.03170707],
          [296247.748197456, 4633735.52663699],
          [296247.246448121, 4633736.09846306],
          [296256.481107703, 4633743.1682759],
          [296256.855867252, 4633742.67161996],
          [296257.337490172, 4633743.03926026],
          [296264.383732139, 4633734.06625588],
          [296264.026023897, 4633733.73391436],
          [296264.387774608, 4633733.1304567],
          [296255.366067528, 4633726.20345278],
          [296254.982315092, 4633726.58571713],
          [296254.482930933, 4633726.19264522]
        ]
      ]
    ],
    'footprint_altitude': [
      126.631,
      144.12100000000001
    ],
    'thumbnail': 'data/sites/162/P1120067.JPG',
    'description_site': 'Pyramidal funerary monument. Composed of opus caementicium, mainly composed of black basalt chips mixed with a mortar full of red tufa grains and fragments as well as chalk pits. Some tiny pieces of white marble inserted. The mass is composed in various horizontal layers not entirely recognizable. From the west side, nine layers can be distinguished. One is the base, the first big protrusion has two layers, the following indent one, the part with the recessions for blocks two more, followed by two final layers. The pieces of basalt are systemized in more or less regular tiers.\r\nThe current form is due to the absence of elements in stone, especially in the base, and of the entire covering with white marble like the Pyramid of Cestius. Cavities made in the higher registers illustrate the practice of insertion of blocks to tighten the structure of the facing.',
    'site_context': 'Funerary',
    'site_interpretation': 'Funerary tower',
    'pointcloud_bbox': [296247.246448120509740, 4633726.192645221017301, 121.484, 296264.387774608097970, 4633743.168275895528495, 144.177],
    'pointcloud': 'http://148.251.106.132:8090/POTREE/PC/SITE/S162/SITE_162_cleaned_ALIGNED_DRIVE_1_V3/SITE_162_cleaned_ALIGNED_DRIVE_1_V3_levels_4/cloud.js',
    'mesh': {
      'mtl_location': 'http://148.251.106.132:8090/RAW/MESH/SITE/CURR/S162/SITE_162_FEB+JUL_MAX_cleaned_ALIGNED_DRIVE_1_V3_SRID_32633/SITE_162_FEB+JUL_MAX_cleaned_ALIGNED.mtl',
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4633735.0145140002,
        'x': 296254.69997949997,
        'ys': 1.0,
        'z': 135.06281300000001,
        'zs': 1.0
      },
      'data_location': 'http://148.251.106.132:8090/RAW/MESH/SITE/CURR/S162/SITE_162_FEB+JUL_MAX_cleaned_ALIGNED_DRIVE_1_V3_SRID_32633/SITE_162_FEB+JUL_MAX_cleaned_ALIGNED_DRIVE_1_V3.obj'
    },
    'reconstruction_mesh': [{
      'id': 1,
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4633734.6418715101,
        'x': 296255.84490104701,
        'ys': 1.0,
        'z': 135.376,
        'zs': 1.0
      },
      'data_location': 'http://148.251.106.132:8090/RAW/MESH/SITE/ARCH_REC/S162/1st_hypothesis_print_162_8bitcolor/1st_hypothesis_print_162.obj'
    }, {
      'id': 2,
      'mtl_location': 'http://148.251.106.132:8090/RAW/MESH/SITE/ARCH_REC/S162/Site162texturen_ALIGNED_SRID_32633/Site162texturen_ALIGNED.mtl',
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4633735.1724659996,
        'x': 296254.56280900002,
        'ys': 1.0,
        'z': 137.7710265,
        'zs': 1.0
      },
      'data_location': 'http://148.251.106.132:8090/RAW/MESH/SITE/ARCH_REC/S162/Site162texturen_ALIGNED_SRID_32633/Site162texturen_ALIGNED.obj'
    }, {
      'id': 3,
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4633734.6418715101,
        'x': 296255.84490104701,
        'ys': 1.0,
        'z': 135.376,
        'zs': 1.0
      },
      'data_location': 'http://148.251.106.132:8090/RAW/MESH/SITE/ARCH_REC/S162/reconstrution/1st_hypothesis_print_162.obj'
    }],
    'objects': [{
      'description_restorations': null,
      'ancient': false,
      'object_material': [{
        'material_type': null,
        'id': 1,
        'material_technique': 'Opus caementicium'
      }, {
        'material_type': null,
        'id': 2,
        'material_technique': null
      }, {
        'material_type': null,
        'id': 3,
        'material_technique': null
      }, {
        'material_type': 'Red',
        'id': 4,
        'material_technique': null
      }],
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4634071.0426368704,
        'x': 295960.45654888899,
        'ys': 1.0,
        'z': 131.64349999999999,
        'zs': 1.0
      },
      'object_interpretation': null,
      'date_specific': null,
      'object_type': 'Edifice',
      'period': null,
      'object_id': 1,
      'in_situ': false,
      'description_object': 'Pyramidal funerary monument. Composed of opus caementicium, mainly composed of black basalt chips mixed with a mortar full of red tufa grains and fragments as well as chalk pits. Some tiny pieces of white marble inserted. The mass is composed in various horizontal layers not entirely recognizable. From the west side, nine layers can be distinguished. One is the base, the first big protrusion has two layers, the following indent one, the part with the recessions for blocks two more, followed by two final layers. The pieces of basalt are systemized in more or less regular tiers.\r\nThe current form is due to the absence of elements in stone, especially in the base, and of the entire covering with white marble like the Pyramid of Cestius. Cavities made in the higher registers illustrate the practice of insertion of blocks to tighten the structure of the facing.',
      'condition': null
    }, {
      'description_restorations': null,
      'ancient': false,
      'object_material': [{
        'material_type': 'Peperino',
        'id': 1,
        'material_technique': null
      }],
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4634071.0426368704,
        'x': 295960.45654888899,
        'ys': 1.0,
        'z': 131.64349999999999,
        'zs': 1.0
      },
      'object_interpretation': null,
      'date_specific': null,
      'object_type': 'Block',
      'period': null,
      'object_id': 2,
      'in_situ': true,
      'description_object': 'At the east side a circular apse (diam. 114 cm) is the lower part of a tunnel made through the core up in an oblique shape versus east. Four tufa block of the original fill are still in situ at the south-east corner.',
      'condition': null
    }, {
      'description_restorations': null,
      'ancient': false,
      'object_material': [{
        'material_type': 'Peperino',
        'id': 1,
        'material_technique': null
      }],
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4634071.0426368704,
        'x': 295960.45654888899,
        'ys': 1.0,
        'z': 131.64349999999999,
        'zs': 1.0
      },
      'object_interpretation': null,
      'date_specific': null,
      'object_type': 'Block',
      'period': null,
      'object_id': 3,
      'in_situ': true,
      'description_object': 'The east side has at the right a rectangular block with a face of 95 x 78 cm, at the left a corner block of 98 x 98 x 100 (top) and 40 (bottom) cm. Next to it at the south side two more blocks are in situ. The middle one measures, as far as visible, 93 x 98 x 55 (top) and 19 (bottom) cm, and has a dowel hole (9 x 10 x 9 cm). The other block is broken off at top and sides and measures 88 x 77 x 68 cm. Thee dowel holes from top to bottom: 9 x 9 x 5cm, 7 x diameter 5 cm, 5 x 6 x 3 cm.\r\nFor Canina the monument belonged to the Villa of the Quintilii, but not to its latest phase under Commodus. This opinion has been followed up to now by most scholar (Quilici, Coarelli; Spera/Mineo who combine it with inscriptions CIL VI 1564-41330). Ripostelli/Marucchi observe the find of a large sphinx (our site 158?) and a head of a large statue. The building would not date to the Republic but to the \u201cd\u00e9clin de l\u2019Empire\u201d.',
      'condition': null
    }]
  }, {
    'id': 13,
    'footprint': [
      [
        [
          [296256.069935701, 4633691.8094286],
          [296254.971269128, 4633693.19429208],
          [296255.380766414, 4633693.51825223],
          [296256.456351441, 4633692.12764678],
          [296256.069935701, 4633691.8094286]
        ]
      ]
    ],
    'footprint_altitude': [
      126.881,
      128.32599999999999
    ],
    'thumbnail': 'data/sites/13/IMG_0040.JPG',
    'description_site': '13.1 > On top, block of white marble. 30,5 x 143,5 x 66 cm. Corners of north-west and south-east sides missing. Chipped and injured. On front and left sides moulding at square angles, forming corner of an entablature. The measurements of the plain lower side are 113 x 31 cm, indicating that the piece grew larger and longer going upwards. Bottom plain except for dowel hole in back (8 x 3 cm). Right side also plain, as is top apart from three shallow dowel holes of 2 \u00bd, 4, and 1 \u00bd cm deep and maybe a fourth one at the proper right side (3 cm deep). The left and front sides show the same moulding becoming wider going upwards and beveled at the top. Moulding from bottom to top: cyma reversa, dentil, cyma recta almost twice the height of the cyma below, half hollow with drooping lower rim, abacus with egg and dart, and finally plain cornice.',
    'site_context': 'Unknown',
    'site_interpretation': 'Unknown',
    'mesh': {
      'mtl_location': 'http://148.251.106.132:8090/RAW/MESH/SITE/CURR/S13/SITE_13_FIRST/SITE_13_MAX_tex.mtl',
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4633692.6671634195,
        'x': 296255.71554512897,
        'ys': 1.0,
        'z': 127.6035,
        'zs': 1.0
      },
      'data_location': 'http://148.251.106.132:8090/RAW/MESH/SITE/CURR/S13/SITE_13_FIRST/SITE_13_MAX.obj'
    },
    'reconstruction_mesh': [],
    'objects': [{
      'description_restorations': null,
      'ancient': false,
      'object_material': [{
        'material_type': null,
        'id': 1,
        'material_technique': null
      }],
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4634071.0426368704,
        'x': 295960.45654888899,
        'ys': 1.0,
        'z': 131.64349999999999,
        'zs': 1.0
      },
      'object_interpretation': null,
      'date_specific': null,
      'object_type': 'Block',
      'period': null,
      'object_id': 1,
      'in_situ': false,
      'description_object': '13.1 > On top, block of white marble. 30,5 x 143,5 x 66 cm. Corners of north-west and south-east sides missing. Chipped and injured. On front and left sides moulding at square angles, forming corner of an entablature. The measurements of the plain lower side are 113 x 31 cm, indicating that the piece grew larger and longer going upwards. Bottom plain except for dowel hole in back (8 x 3 cm). Right side also plain, as is top apart from three shallow dowel holes of 2 \u00bd, 4, and 1 \u00bd cm deep and maybe a fourth one at the proper right side (3 cm deep). The left and front sides show the same moulding becoming wider going upwards and beveled at the top. Moulding from bottom to top: cyma reversa, dentil, cyma recta almost twice the height of the cyma below, half hollow with drooping lower rim, abacus with egg and dart, and finally plain cornice.',
      'condition': 'Damaged'
    }, {
      'description_restorations': null,
      'ancient': false,
      'object_material': [{
        'material_type': 'Peperino',
        'id': 1,
        'material_technique': null
      }],
      'osg_position': {
        'h': 0.0,
        'p': 0.0,
        'r': 0.0,
        'xs': 1.0,
        'y': 4634071.0426368704,
        'x': 295960.45654888899,
        'ys': 1.0,
        'z': 131.64349999999999,
        'zs': 1.0
      },
      'object_interpretation': null,
      'date_specific': null,
      'object_type': 'Block',
      'period': null,
      'object_id': 2,
      'in_situ': false,
      'description_object': 'Tufa block underneath is part of an architectonic structure. 30 x 170 x 48 cm. All surfaces in sight are plain except for west side, which is roughly worked with an axe. Bottom invisible. It looks similar to the foundation blocks in Site 25.',
      'condition': null
    }]
  }]
).value('defaultSitesGeoJSON', {
    'type': 'FeatureCollection',
    'features': [{
      'type': 'Feature',
      'id': 162,
      'geometry': {
        'type': 'MultiPolygon',
        'coordinates': [
          [
            [
              [296254.482930933, 4633726.19264522],
              [296247.357277832, 4633735.03170707],
              [296247.748197456, 4633735.52663699],
              [296247.246448121, 4633736.09846306],
              [296256.481107703, 4633743.1682759],
              [296256.855867252, 4633742.67161996],
              [296257.337490172, 4633743.03926026],
              [296264.383732139, 4633734.06625588],
              [296264.026023897, 4633733.73391436],
              [296264.387774608, 4633733.1304567],
              [296255.366067528, 4633726.20345278],
              [296254.982315092, 4633726.58571713],
              [296254.482930933, 4633726.19264522]
            ]
          ]
        ]
      }
    }, {
      'type': 'Feature',
      'id': 13,
      'geometry': {
        'type': 'MultiPolygon',
        'coordinates': [
          [
            [
              [296256.069935701, 4633691.8094286],
              [296254.971269128, 4633693.19429208],
              [296255.380766414, 4633693.51825223],
              [296256.456351441, 4633692.12764678],
              [296256.069935701, 4633691.8094286]
            ]
          ]
        ]
      }
    }],
    'crs': {
      'type': 'name',
      'properties': {
        'name': 'urn:ogc:def:crs:EPSG::32633'
      }
    }
  });
